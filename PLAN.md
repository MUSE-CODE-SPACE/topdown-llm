...
- 2026-06-22 — EP8 DONE (cost/speed: pickModel haiku/sonnet + prompt caching cache_control). tag ep8 + folder + handout. Video ~1:18 (build-ep8-rich.mjs from ep7 template). Clips models.mp4 (model badges). NEXT EP9 deploy.
- 2026-06-22 — EP9 DONE (deploy: DEPLOY.md GitHub→Vercel guide; build verified). tag ep9 + folder + handout. Video ~1:14 (build-ep9-rich.mjs from ep8). Build terminal clip (real npm run build ✓). NEXT EP10 capstone (last).
- 2026-06-22 — EP10 DONE (capstone: Top-Down Tutor signature). SERIES COMPLETE EP0~EP10 (11편). All: app+tag+episodes folder+handout+rich video+thumb+srt+upload texts+Telegram. Tutor clip + recap montage + yesterday-me ending. build-ep10-rich.mjs bespoke.
